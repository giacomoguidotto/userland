...
"$release_dir/assemble-site.sh" "$work/cache" v1.2.3 "$work/site"

npm exec --prefix "$release_dir/cloudflare" -- wrangler dev \
  --config "$release_dir/cloudflare/wrangler.jsonc" \
  --assets "$work/site" \
  --ip 127.0.0.1 \
  --port "$port" \
  --local \
...
"$release_dir/verify-endpoint.sh" "$origin" "$work/cache/v1.2.3/bootstrap" root
"$release_dir/verify-endpoint.sh" "$origin/v1.2.3" "$work/cache/v1.2.3/bootstrap" version
