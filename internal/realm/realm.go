...
// HasAttachments reports whether a previous release left any realm bindings.
// It is separate from Options so an empty catalog can still clean up old
// attachments during a normal sync.
func (m Manager) HasAttachments() (bool, error) {
...
// Purge removes all realm projections after the catalog has been disabled.
// It never deletes a checkout or an unrelated file. Generated .envrc files
// are removed only when their marker proves Userland owns them.
func (m Manager) Purge(ctx context.Context) error {
	...
	for _, attached := range attachments {
		...
		if contents, readErr := os.ReadFile(link); readErr == nil && strings.HasPrefix(string(contents), "# Generated by userland realm. Do not edit.\n") {
	...
	if err := m.writeAttachments(nil); err != nil {
		return err
	}
	return m.writeRealmConfigurations(nil)
}
