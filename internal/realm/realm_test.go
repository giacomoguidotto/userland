...
func TestRealmSSHConfigurationFollowsAttachmentLifecycle(t *testing.T) {
	fixture := newFixture(t)
	...
	if _, err := manager.Remove(context.Background(), "work"); err != nil {
		t.Fatal(err)
	}
	detached := readFile(t, sshRealms)
	if strings.Contains(detached, "realm-remote") || detached != "# Generated by userland realm. Do not edit.\n" {
		t.Fatalf("detached realm left its SSH projection active: %q", detached)
	}
}
