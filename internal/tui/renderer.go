...
func (r Renderer) Usage() {
	...
	fmt.Fprintf(r.out, "%s  realm add <name>\n", margin)
	fmt.Fprintf(r.out, "%s  realm add <repository> <path>\n", margin)
	fmt.Fprintf(r.out, "%s            Attach optional private configuration\n", margin)
	fmt.Fprintf(r.out, "%s  realm remove <name-or-path>\n", margin)
	fmt.Fprintf(r.out, "%s            Detach configuration without deleting its checkout\n\n", margin)
