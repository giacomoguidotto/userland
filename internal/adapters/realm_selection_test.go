...
func TestRealmSelectionPurgesAttachmentsAfterCatalogIsCleared(t *testing.T) {
	base := t.TempDir()
	root := filepath.Join(base, "root")
	home := filepath.Join(base, "home")
	state := filepath.Join(base, "state")
	mount := filepath.Join(home, "dev", "old-realm")
	if err := os.MkdirAll(filepath.Join(root, "cfg"), 0o700); err != nil {
		t.Fatal(err)
	}
	...
	if err := os.WriteFile(filepath.Join(root, "cfg", "realms.csv"), []byte("name,repository,configuration_path,default_path,branch,mode\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(state, "realms.csv"), []byte("name,path\nold,~/dev/old-realm\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(mount, ".envrc"), []byte("# Generated by userland realm. Do not edit.\nexport USERLAND_REALM='old'\n"), 0o600); err != nil {
		t.Fatal(err)
	}
	env := platform.NewEnvironment([]string{
		"USERLAND_ROOT=" + root,
	...
	if code := realmSelection(invocation, Apply); code != 0 {
		t.Fatalf("purge returned %d: %#v", code, invocation.Events)
	}
	if _, err := os.Stat(filepath.Join(mount, ".envrc")); !os.IsNotExist(err) {
		t.Fatalf("managed activation survived purge: %v", err)
	}
	if got, err := os.ReadFile(filepath.Join(state, "realms.csv")); err != nil || string(got) != "name,path\n" {
		t.Fatalf("attachment map was not cleared: %q %v", got, err)
	}
}
