...
userland_require_schema() {
  ...
  userland_schema_file=$USERLAND_ROOT/state/schema-version
...
userland_run_adapters() {
  ...
  while IFS= read -r userland_adapter_name; do
    case "$userland_adapter_name" in '' | '#'*) continue ;; esac
    userland_adapter=$USERLAND_ROOT/libexec/userland/adapters/$userland_adapter_name.sh
    [ -x "$userland_adapter" ] || {
  ...
  done <"$USERLAND_ROOT/state/adapters"
